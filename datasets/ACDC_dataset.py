...
class ACDCReader(MIBasicTrain):
    ...
    def load_data(self, pid):
        ...
        pid_labels = np.concatenate(pid_labels, axis=0)
        if pid_data.ndim == 3:
            pid_data = pid_data[:, :, :, np.newaxis]
        if pid_labels.ndim == 3:
            pid_labels = pid_labels[:, :, :, np.newaxis]
        self.norm_paras[pid] = pid_data_ranges
        return pid_data, pid_labels
...
class ACDCSegDataset(ACDCReader, MIBasicTrain, MIBasicValid):
    ...
    def __init__(self, paras, patient_ids=None):
        super(ACDCSegDataset, self).__init__()
